...
def test_merge_referendum_and_area():
    ...
    assert (
        referendum_and_areas.shape == referendum_and_areas.dropna().shape
...
def test_compute_referendum_result_by_regions():
    ...
    assert (
        referendum_result_by_regions.loc["Normandie", "Abstentions"] == 426_075
    )
    assert (
        referendum_result_by_regions.loc["Grand Est", "Choice A"] == 1_088_684
    )
