...
def merge_regions_and_departments(regions, departments):
    ...
    """
    dfjoin = departments.merge(
        regions, left_on="region_code", right_on="code", suffixes=("_x", "_y")
    )
    dfjoin = dfjoin[["region_code", "name_y", "code_x", "name_x"]]
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    referendum["Department code"] = referendum["Department code"].str.zfill(
        2
    )  # pr avoir 01 et pas 1

    dfjoin = referendum.merge(
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    dfgeo = geo.merge(referendum_result_by_regions, left_on="code", right_on="code_reg")
    dfgeo.plot(column="ratio", legend=True)
    return dfgeo
