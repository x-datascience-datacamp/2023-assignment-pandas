...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    """
    regions_and_departments['code_dep2'] = regions_and_departments[
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    """
    map_df = gpd.read_file('data/regions.geojson')
    df = referendum_result_by_regions.reset_index()
    df_plot = pd.merge(
        map_df, df, how='inner',
        left_on='code', right_on='code_reg')
    df_plot['ratio'] = df_plot['Choice A'] / (
        df_plot['Choice A'] +
        df_plot['Choice B'])
    return df_plot[['code', 'name_reg', 'geometry', 'ratio']]
