...
def merge_regions_and_departments(regions, departments):
    ...
    """
    merged = pd.merge(departments, regions, left_on="region_code",
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    referendum["Department code"].replace(
        {str(i): '0'+str(i) for i in range(1, 10)}, inplace=True
    )
    ref_areas = pd.merge(regions_and_departments, referendum,
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    ballots = merged["Choice A"] + merged["Choice B"]
    merged["ratio"] = merged["Choice A"] / ballots
