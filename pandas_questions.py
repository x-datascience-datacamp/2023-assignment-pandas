...
def merge_regions_and_departments(regions, departments):
    ...
    mergeddf = pd.merge(regions[['code_reg', 'name_reg']],
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    agg = referendum_and_areas.groupby(["code_reg", "name_reg"])
    agg = agg.aggregate({'Registered': 'sum',
                         'Abstentions': 'sum',
                         'Null': 'sum',
                         'Choice A': 'sum',
                         'Choice B': 'sum'})
    return agg.reset_index("name_reg")
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    g = g.merge(referendum_result_by_regions,
                left_on="code",
                right_on="code_reg")
