...
def merge_regions_and_departments(regions, departments):
    ...
    """
    df1 = pd.merge(regions[["code", "name"]],
                   departments[["region_code", "code", "name"]],
                   left_on=["code"], right_on=["region_code"])
    df1.drop("region_code", axis=1, inplace=True)
    df1.columns = ['code_reg', 'name_reg', 'code_dep', 'name_dep']
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    res = pd.merge(referendum_result_by_regions, geo,
                   right_on='code', left_index=True)
    res['ratio'] = res.apply(lambda x: x['Choice A'] /
                             (x['Choice A']+x['Choice B']), axis=1)
