...
def merge_regions_and_departments(regions, departments):
    ...
    dep = departments.rename(columns={"code": "code_dep", "region_code": "code_reg", "name": "name_dep"})
    df = pd.merge(reg[['code_reg', 'name_reg']],
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    ref_a = referendum_and_areas[['code_reg', 'name_reg', 'Registered', 'Abstentions', 'Null', 'Choice A', 'Choice B']]
    df = ref_a.groupby(['code_reg']).agg({'code_reg': 'first',
                                                          'name_reg': 'first',
                                                          'Registered': 'sum',
                                                          'Abstentions': 'sum',
                                                          'Null': 'sum',
                                                          'Choice A': 'sum',
                                                          'Choice B': 'sum'})
    df.drop("code_reg", axis=1, inplace=True)
    return df
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    df = pd.merge(gdf, referendum_result_by_regions,
                    on='code_reg', how='inner')
